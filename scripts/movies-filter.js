...
const filterMovies = (USERS, MOVIES, fromDate, toDate, rate, userId) => {
  // filter by date
  const filterMoviesByDate = MOVIES.filter((movie) => {
      const movieDate = new Date(movie.watched);
      return movieDate >= new Date(fromDate) && movieDate <= new Date(toDate);
  });

  // filter by rate
  const filterMoviesByRate = filterMoviesByDate.filter((movie) => movie.rate > rate);

  // filter by userId optional
  let filteredMovies;
  if (userId) {
      filteredMovies = filterMoviesByRate.filter((movie) => movie.userId === userId);
  } else {
      filteredMovies = filterMoviesByRate;
  }
...
form.addEventListener('submit', (e) => {
  ...
  const filteredMovies = filterMovies(USERS, MOVIES, fromDate, toDate, rate, userId);
...
if (filteredMovies.length > 0) {
  filteredMovies.forEach((movie) => {
    ...
    idParagraph.appendChild(idText);
    movieDiv.appendChild(idParagraph);

    const usernameParagraph = document.createElement("p");
    const usernameText = document.createTextNode(`Username: ${user.username}`);
    usernameParagraph.appendChild(usernameText);
    movieDiv.appendChild(usernameParagraph);

    const emailParagraph = document.createElement("p");
    const emailText = document.createTextNode(`Email: ${user.email}`);
    emailParagraph.appendChild(emailText);
    movieDiv.appendChild(emailParagraph);

    const addressParagraph = document.createElement("p");
    const addressText = document.createTextNode(`Full Address: ${user.address.street} - ${user.address.city}`);
    addressParagraph.appendChild(addressText);
    movieDiv.appendChild(addressParagraph);

    const companyParagraph = document.createElement("p");
    const companyText = document.createTextNode(`Company: ${user.company.name}`);
    companyParagraph.appendChild(companyText);
    movieDiv.appendChild(companyParagraph);

    const movieTitleParagraph = document.createElement("p");
    const movieTitleText = document.createTextNode(`Movie: ${movie.title}`);
    movieTitleParagraph.appendChild(movieTitleText);
    movieDiv.appendChild(movieTitleParagraph);

    const rateParagraph = document.createElement("p");
    const rateText = document.createTextNode(`Rate: ${movie.rate}`);
    rateParagraph.appendChild(rateText);
    movieDiv.appendChild(rateParagraph);
  ...
  noMoviesParagraph.classList.add("noMovies")
  const noMoviesText = document.createTextNode("No movies found.");

  noMoviesParagraph.appendChild(noMoviesText);
  resultDiv.appendChild(noMoviesParagraph);
}
